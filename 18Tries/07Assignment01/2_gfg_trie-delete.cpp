...
public:
    // recursive solution
    void deleteKey(trie_node_t *cur, char word[], int pos = 0) {
        if (!cur) return;
        if (word[pos] == '\0') {
            if (cur->value) cur->value = 0;
            return;
        }
        deleteKey(cur->children[word[pos]-'a'], word, pos+1);
    }

    // iterative solution
    void deletion(trie_node_t *cur, char word[]) {
        int cnt{0};
        while (word[cnt] != '\0') {
            if (!cur->children[word[cnt]-'a']) return;
            cur = cur->children[word[cnt++]-'a'];        
        }
        cur->value = 0;
    }
};
